...
func TestMigrations(t *testing.T) {
	...
	// Verify tables exist
	tables := []string{"rules", "pending_requests", "request_logs", "http_transactions", "pending_http_requests", "schema_migrations", "conversations", "turns", "conversation_processing_state"}
	...
	if count != 8 {
		t.Errorf("expected 8 migration versions, got %d", count)
	}
}
