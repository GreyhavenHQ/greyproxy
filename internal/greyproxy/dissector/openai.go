	...
	for _, raw := range body.Input {
		...
		case probe.Type == "function_call":
			...
			if json.Unmarshal([]byte(fc.Arguments), &argsMap) == nil {
				cb.ToolSummary = extractOpenAIToolSummary(fc.Name, argsMap)
			}
				...
				if toolCalls[i].ToolUseID != "" {
					...
					if matchToolCallID(toolCalls[i].ToolUseID, itemID) || i == len(toolCalls)-1 {
						...
						if json.Unmarshal([]byte(args), &argsMap) == nil {
							toolCalls[i].ToolSummary = extractOpenAIToolSummary(toolCalls[i].Tool, argsMap)
						}
						break
					}
				}
...
// extractOpenAIToolSummary produces a human-readable summary from tool arguments.
// OpenAI tool names are lowercase versions of the same tools (bash, read, glob, etc.).
func extractOpenAIToolSummary(toolName string, input map[string]any) string {
